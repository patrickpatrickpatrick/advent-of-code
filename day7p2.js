const input = `muted coral bags contain 1 bright magenta bag, 1 dim aqua bag.
...
const collapsedBags = input.split('\n').reduce((bags, bag) => {
	const splitBag = bag.split(' contain ');
	return {
		...bags,
		[splitBag[0].replace('bags', 'bag')]: splitBag[1].split(', ').reduce((bags, bag) => ({
			...bags,
			...(
				bag.split(' ').slice(1).join(' ') === 'other bags.' ? {} : {
					[bag.split(' ').slice(1).join(' ').replace('.', '').replace('bags', 'bag')]: bag.split(' ')[0]
				}
			)
		}), {})
	}
...
const bags = Object.keys(collapsedBags);
let validBags = 0;

const bagChecker = (bag, amount = 1) => {
	const bagChildren = collapsedBags[bag];
	if (Object.keys(bagChildren).length) {
		validBags += (amount * Object.values(bagChildren).reduce((sum, num) => parseInt(sum) + parseInt(num)));
		Object.keys(bagChildren).forEach(bag => bagChecker(bag, amount * bagChildren[bag]));
	}
}

bagChecker('shiny gold bag');
