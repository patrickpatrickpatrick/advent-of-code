...
const bagChecker = (bag) => {
	const bagChildren = collapsedBags[bag];
	if (Object.keys(bagChildren).length) {
		if ((new Set(Object.keys(bagChildren))).has('shiny gold bag')) {
			return 1;
		} else {
			return Object.keys(bagChildren).map(bag => bagChecker(bag)).reduce((sum, num) => sum + num) > 0 ? 1 : 0;
		}
	} else {
		return 0;
	}
